...
const fetchMe = authToken =>
  fetchAPI(authToken, "me").then(res => res.results[0]);

const firebaseConfig = JSON.parse(process.env.REACT_APP_FIREBASE_AUTH_CONFIG);
const firebaseConfigDflt = JSON.parse(
  process.env.REACT_APP_FIREBASE_DFLT_CONFIG
);
...
export const firestore_ref = firebase.firestore();
  ...
  render() {
    ...
    return (
      <div>
        <Modal
          size="mini"
          open={this.props.isOpenLoginForm}
          closeIcon
          onClose={this.props.cancelLoginForm}
        >
